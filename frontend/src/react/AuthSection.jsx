...
export default function AuthSection(props) {
  const handleClick = (e) => {
    if (e.target.parentElement) {
      e.target.parentElement.querySelector(".form-field").classList.toggle("hidden")
    }
  }
  ...
  if(props.mode == "logged_in"){
    ...
    <form ref={profileBtnRef} className="form-field hidden" onSubmit={(e) => props.handleFormSubmit(e, "login")}>
      ...
      <p>Alternatively, <span onClick={() => props.handleAuthState("register")}>register.</span></p>
    </form>

  } else if(props.mode == "register"){
    form = 
    <form ref={profileBtnRef} className="form-field hidden" onSubmit={(e) => props.handleFormSubmit(e, "register")}>
      <h3>Registration</h3>
      ...
      <p>Alternatively, <span onClick={() => props.handleAuthState("login")}>log in.</span></p>
    </form>
  }
